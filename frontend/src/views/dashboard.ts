...
export function renderDashboard(): HTMLElement {
  ...
  wrap.className = 'relative m-0 flex flex-col items-center justify-center text-center z-10';
  ...
  async function loadTop5() {
    try {
      const data = await fetchJSON<LeaderRow[]>(`${API_BASE}/leaderboard?limit=5`);
      renderLeaders(data);
